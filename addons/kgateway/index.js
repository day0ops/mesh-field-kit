...
export class KgatewayFeature extends AddonFeature {
  constructor(name, config = {}) {
    ...
    this.telemetryGatewayNamespace = config.telemetryGatewayNamespace || config.gateway?.namespace || this.namespace
    ...
    this.tracesCollectorNamespace = config.tracesCollectorNamespace || this.telemetryNamespace || 'telemetry'
    this.logsCollectorName = config.logsCollectorName || 'opentelemetry-collector-logs'
    this.logsCollectorNamespace = config.logsCollectorNamespace || this.telemetryNamespace || 'telemetry'
  }

  validate() {
    if (this.enterprise && !process.env.ENTERPRISE_KGATEWAY_LICENSE) {
      throw new Error('ENTERPRISE_KGATEWAY_LICENSE environment variable is required for kgateway Enterprise')
    }
    return true
  }
  ...
  async deploy() {
    ...
    const mainArgs = [
      'upgrade', '-i', this.mainRelease,
      `${this.registry}/${this.mainRelease}`,
      '-n', this.namespace,
      '--version', this.version,
      '--wait',
      '--timeout', '5m',
      '--values', path.join(CONFIG_DIR, 'values.yaml'),
      ...ctxArgs,
    ]
    if (this.enterprise) {
    ...
    await KubernetesHelper.waitForDeployment(
      this.namespace, this.mainRelease, 120, this.spinner, this.kubeContext
    )

    if (this.gateway) {
      await this.createGatewayResource(this.enterprise ? ENTERPRISE_GATEWAY_CLASS : OSS_GATEWAY_CLASS)
    }
  ...
  async createGatewayResource(gatewayClassName) {
    ...
    if (this.ambientEnabled) {
      this.log(`Applying GatewayParameters "${this.gatewayParametersName}" (bypass-inbound-capture)`)
  ...
  async applyTelemetryPolicies() {
    this.log(`Applying OTel telemetry policies for gateway '${this.telemetryGatewayName}'...`, 'info')
    ...
    await this.applyResource({
      ...
      kind: 'ReferenceGrant',
      metadata: { name: 'allow-otel-collector-logs-access', namespace: this.logsCollectorNamespace },
      spec: {
    ...
    await this.applyResource({
      apiVersion: 'gateway.networking.k8s.io/v1beta1',
      kind: 'ReferenceGrant',
      metadata: { name: 'allow-otel-collector-traces-access', namespace: this.tracesCollectorNamespace },
      spec: {
        from: [{ group: 'gateway.kgateway.dev', kind: 'ListenerPolicy', namespace: gatewayNs }],
  ...
  async cleanup() {
    ...
    if (this.gateway) {
      await this.deleteResource('gateway', this.gateway.name, this.gateway.namespace, this.kubeContext)
      if (this.ambientEnabled) {
        const parametersKind = this.enterprise ? 'enterprisekgatewayparameters' : 'gatewayparameters'
        await this.deleteResource(parametersKind, this.gatewayParametersName, this.gateway.namespace, this.kubeContext)
      }
    }
    ...
    if (this.telemetryGatewayName) {
      ...
      await this.deleteResource('referencegrant', 'allow-otel-collector-logs-access', this.logsCollectorNamespace, this.kubeContext)
      await this.deleteResource('referencegrant', 'allow-otel-collector-traces-access', this.tracesCollectorNamespace, this.kubeContext)
    }

    for (const release of [this.mainRelease, this.crdsRelease]) {
      try {
        await KubernetesHelper.helm(['uninstall', release, '-n', this.namespace, ...helmCtxArgs], { spinner: this.spinner })
      } catch (err) {
        if (!/not found|no deployed releases/i.test(err.message)) throw err
      }
    }
    await KubernetesHelper.kubectl([...kubectlCtxArgs, 'delete', 'namespace', this.namespace, '--ignore-not-found=true'])
