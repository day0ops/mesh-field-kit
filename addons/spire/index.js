 ...
 *     }
 *   },
 *   multiRoot: boolean,         // Default: false. self-signed only. When true, SPIRE keeps
 *                               // its own independent root instead of reusing istiod cacerts'
 *                               // shared root, and both roots are federated into each other's
 *                               // trust bundle so peers issued off either root are trusted.
 *   distinctRoots: boolean,     // Default: false. self-signed only. Implies multiRoot. Each
 *                               // cluster gets its own independent root, pre-generated by
 *                               // SpireRootManager before any cluster installs (required -
 *                               // deploy() throws if no pre-generated root is found). Every
...
export class SpireFeature extends AddonFeature {
  ...
  /**
   * Extract the last certificate in a PEM chain (the root). SPIRE's bundle_file_path
   * must publish only the root - peers already present their own intermediate, so a
   * root-only bundle is a complete trust anchor.
   */
  ...
  async #prepareSelfSignedCerts(context) {
    ...
    // Reuse istiod's cacerts root when one already exists (multicluster installs),
    // so SPIRE's intermediate and istiod's are mutually trusted without federation.
    // Falls back to a SPIRE-only root for single-cluster installs.
    ...
    // Full chain (intermediate + root) - used to federate SPIRE's trust into Istio's
    // cacerts secret (see #federateWithIstioCA), independent of what SPIRE itself
    // publishes via bundle_file_path (root only, see #extractRootCert).
    const caBundle = intermediateCert + rootCert;
    writeFileSync(caBundlePath, caBundle);

    // Multi-root mode: fold istiod's cacerts root into SPIRE's own published bundle too,
    // so real ambient workloads (via delegated_api_client.rs, which allows multiple trust
    // bundle authorities) trust istiod-issued peers without sharing a common root.
    ...
    // Distinct-roots mode: every peer cluster's root is an additional, independent
    // trust anchor - not cross-signed, just listed alongside ours (see class JSDoc).
    ...
    // Federate into cacerts: own chain + peer roots (peers already present their own
    // intermediate during the handshake, so root-only is enough for them).
    return peerRoots.length > 0 ? `${caBundle.trimEnd()}\n${peerRoots.join('\n')}` : caBundle;
  }

  /**
   * Every other pre-generated trust domain under SPIRE_DISTINCT_ROOTS_DIR is a peer
   * cluster's independent root (see SpireRootManager in multicluster.js).
   */
  #discoverPeerRoots() {
  ...
  /**
   * Fetch istiod's cacerts root-cert.pem, or null if the secret doesn't exist.
   */
  ...
  async cleanup() {
    ...
    // 4. Force-delete any lingering pods before deleting the namespace. Pods left behind
    // by the uninstalled charts (e.g. spiffe-oidc-discovery-provider, which finishes as
    // Completed) routinely get stuck and block namespace finalization indefinitely -
    // kubectl delete namespace then hangs forever on "unexpected items still remain ...
    // for gvr: /v1, Resource=pods", which does not clear on its own in practice.
    try {
      await KubernetesHelper.kubectl(
        [
          ...ctxArgs,
